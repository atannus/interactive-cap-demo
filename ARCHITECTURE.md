...
- **CP systems**: sacrifice availability — reject or block operations on nodes that cannot confirm consistency with other nodes
- **AP systems**: sacrifice consistency — accept writes and serve reads on every node, accepting that different nodes will temporarily see different data
